...
class SpanLabeledTextDataset(Dataset):
    ...
    def _prepare_data(self):
        ...
        print('\n\n prepared data, ', 
            ...
            '\n\n >>> mapping should match huggingface config ==>> tag_idx_map', tag_idx_map,
...
class TransformersBasedTagger(LabelStudioMLBase):
    ...
    def predict(self, tasks, **kwargs):
        ...
        for batch in tqdm(predict_loader, desc='Prediction'):
            print('\n\n >> this is what a batch looks like in predict', batch)
            inputs = {
            ...
            for max_preds, argmax_preds, mask_tokens, token_start_map, string in zip(
                ...
                for label, group in groupby(zip(preds, starts, scores), key=lambda i: re.sub('^(B-|I-)', '', i[0])):
                    print('\n\n >> in predict for loop, this is NER label', label, 'and this is group', group)
    ...
    def fit(
        self, completions, workdir=None, model_type='bert', pretrained_model='mrm8488/bert-spanish-cased-finetuned-pos-16-tags',
        batch_size=32, learning_rate=5e-5, adam_epsilon=1e-8, num_train_epochs=2, weight_decay=0.0, logging_steps=1,
