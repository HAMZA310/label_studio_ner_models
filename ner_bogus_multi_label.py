...
print('\n\n\n\n >> before data class')
logger.debug('>> before data class with logger')
...
class SpanLabeledTextDataset(Dataset):
    ...
    def __init__(
        ...
        print('\n\n\n\n >> entered data class')
        logger.debug('>> entered data class with logger')
        ...
        if tag_idx_map is None:
            ...
            print('\n\n >> creatd tag_idx_mapping using self.prepare data', self.tag_idx_map)
        else:
    ...
    def _prepare_data(self):
        ...
        tag_idx_map = {'O': 0} # Why do we need this tag?
        for text, spans in zip(self.list_of_strings, self.list_of_spans):

            # list_of_strings = ['I love you', 'I hate you',...]
            # list_of_spans = [  [{'label': 'I-PER', 'start'..},...]   ..., [], ]
        ...
        print('\n\n prepared data, ', 
            ...
            'list_of_token_idx_maps', list_of_token_idx_maps)
...
print('\n\n\n\n >> before transformers class')

class TransformersBasedTagger(LabelStudioMLBase):

    def __init__(self, **kwargs):
        print('\n\n\n\n >> instantiating transformers class')
        logger.debug('>> instantiating transformers class with logger')
        super(TransformersBasedTagger, self).__init__(**kwargs)

        assert len(self.parsed_label_config) == 1
        self.from_name, self.info = list(self.parsed_label_config.items())[0]
        print('\n\n>> this is self.info,', self.info)
    ...
    def predict(self, tasks, **kwargs):
        print('\n\n\n\n >> entered predict in transformers')
        ...
        for batch in tqdm(predict_loader, desc='Prediction'):
            ...
            print('\n\n >> predicted logist by model', logits)
    ...
    def get_spans(self, completion):
        print('\n\n\n\n >> entered get_spans in transformers')
        print('\n\n >>item received in get_spans', completion)
        spans = []
    ...
    def fit(
        self, completions, workdir=None, model_type='bert', pretrained_model='dslim/bert-base-NER',
        batch_size=32, learning_rate=5e-5, adam_epsilon=1e-8, num_train_epochs=3, weight_decay=0.0, logging_steps=1,
        ...
        **kwargs
        ...
        texts, list_of_spans = [], []
        print('\n\n >> this is completions given to fit method in ner', completions)
        ...
        print('\n\n Adding the following number of labels to the model config file: ',train_set.num_labels)
        print('if you are loading from a pretrained checkpoit (huggingface hub), this number of labels should match')
        ...
        print('\n\n\n >>> this mapping based on trains set will be used to create label map', train_set.tag_idx_map.items())
