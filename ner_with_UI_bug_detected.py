...
class TransformersBasedTagger(LabelStudioMLBase):
    ...
    def predict(self, tasks, **kwargs):
        ...
        for batch in tqdm(predict_loader, desc='Prediction'):
            ...
            for max_preds, argmax_preds, mask_tokens, token_start_map, string in zip(
                ...
                result = []
                print("Bug causing Label mismatch is here in code in regex")
                ### BUG FOUND 
                ## re.sub('^(B-|I-)', '', i[0])):
                ## Removes B and I from labels which causes
                ## mismatch in the labels.
                ### 
                for label, group in groupby(zip(preds, starts, scores), key=lambda i: i[0]):
    ...
    def fit(
        ...
        **kwargs
        ...
        model_to_save.save_pretrained(saved_model_dir)
